...
export function qty(d){sel.qty=Math.max(1,sel.qty+ +d);$('ppQty').textContent=sel.qty;price()}

/* ===== CONFIGURATORE PRODOTTO =====
   Solo la taglia, con moltiplicatore di prezzo.
   sel.sizeIdx → indice della taglia scelta
   sel.sizeMul → moltiplicatore prezzo (es. 1.0 = base, 1.4 = +40%)

   Il MATERIALE non si sceglie qui. Prima la pagina proponeva anche i materiali
   usati da ALTRI prodotti della stessa categoria: su un prodotto in Metallo
   comparivano Legno, Plexiglass e PLA, che per quel prodotto non esistono e non
   cambiavano nulla nell'ordine. Il materiale è quello impostato nell'Admin ed è
   già scritto sopra la scheda. */
const SIZE_MULS={
...
function renderConfigurator(){
  ...
  cfg.style.display=showSize?'block':'none';
  /* --- selezione taglia --- */
  if(showSize){
...
/* Prezzo lineare: prezzo unitario × quantità.
   Lo sconto a scaglioni è stato tolto insieme alla tabella che lo spiegava:
...
export function addFromPP(){
  ...
  const note=(sel.sizeIdx!==undefined&&cur.misure)?(cur.misure[sel.sizeIdx]?.[1]||''):'';
  addToCart(cur.id,sel.qty,matLabel,note,unit());
}
